...
export function FlameStatus() {
  ...
  const { refetch:refetchCurrentFanning, data: currentFanning } = useKeepAliveGameCurrentFanning();
  const { refetch:refetchCurrentFuel, data: currentFuel } = useKeepAliveGameCurrentFuel();

  function balancePercentage(a, b) {
    if (a === 0 && b === 0) {
      return 0; // 0% when both numbers are zero
    }

    const difference = Math.abs(a - b);
    const max = Math.max(a, b);
    const imbalance = (difference / max) * 100;
  ...
  const unwatch = watchBlockNumber(
    ...
    () => {
      refetchFlameSize();
      refetchCurrentFanning();
      refetchCurrentFuel();
      setImbalance( balancePercentage(Number(currentFanning), Number(currentFuel)));
      setLayers(Number(flameSize) > 0 ? Math.ceil(Math.log10(Number(flameSize))) : 0);
    },
  )
        ...
        <div className="balanceMeter">
          <div className="balanceMeterChartWrapper">
            ...
            <div className="balanceMeterChart">
              <div className="balanceMeterKnob" style={{ left: `${Number(currentFuel) / (Number(currentFanning) + Number(currentFuel)) * 100 }%` }}></div>
              <div className="balanceMeterLine"></div>
            </div>
            <div className="balanceMeterLabel">🪵 { `${currentFuel}` }</div>
          </div>
          <div className="balanceMeterAssessment">
            { imbalance < balanceThreshold ? '👍 Balanced' : '🙅 Unbalanced' }
          </div>
        </div>
      }
